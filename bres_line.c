...
/*   Updated: 2019/10/07 17:42:09 by dbubnov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (x > 1200 + fgroup->move_x  + fgroup->land_width 
        || y > 1200 + fgroup->move_y + fgroup->land_height 
...
void bres_line(float x0,  float y0, float x1, float y1, t_fgroup *fgroup)
{
    ...
    {  
        mlx_pixel_put(fgroup->mlx_ptr, fgroup->win_ptr, x0 + fgroup->move_x, y0 + fgroup->move_y, fgroup->color);
        x0 += step_x;
        y0 += step_y;   
    }
}